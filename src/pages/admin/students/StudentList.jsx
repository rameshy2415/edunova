  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Total",        value: STUDENTS.length,                                           color: "bg-cobalt-light text-cobalt" },
          ...
          { label: "Avg att.",     value: Math.round(STUDENTS.reduce((a, s) => a + s.attendance, 0) / STUDENTS.length) + "%", color: "bg-amber-light text-amber" },
        ].map((s) => (
      ...
      <Card padding={false}>
        ...
        <div className="flex flex-wrap items-center gap-3 p-4 border-b border-ink/5">
          ...
          <select value={classF}  onChange={(e) => setClassF(e.target.value)}
            ...
            {["All", ...CLASSES].map((o) => <option key={o}>{o}</option>)}
          </select>

          <select value={feeF} onChange={(e) => setFeeF(e.target.value)}
            className="bg-parchment border border-ink/10 rounded-xl px-3 py-2 text-sm text-ink outline-none cursor-pointer">
            {["All", "Paid", "Partial", "Overdue"].map((o) => <option key={o}>{o}</option>)}
          </select>

          <select value={statusF} onChange={(e) => setStatusF(e.target.value)}
            className="bg-parchment border border-ink/10 rounded-xl px-3 py-2 text-sm text-ink outline-none cursor-pointer">
            {["All", "Active", "Inactive", "Suspended"].map((o) => <option key={o}>{o}</option>)}
          </select>
            ...
            <tbody>
              {filtered.map((s) => (
                <tr key={s.id} className="hover:bg-parchment/40 transition-colors group">
                  ...
                  <Td><Badge variant={FEE_VARIANT[s.fees]}>{s.fees}</Badge></Td>
                  <Td><Badge variant={STATUS_VARIANT[s.status]}>{s.status}</Badge></Td>
