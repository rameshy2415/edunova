...
/* ── Mock fee payments ── */
const PAYMENTS = [
  { date: "12 Apr 2026", desc: "Q1 Tuition + Transport", amount: 13500, method: "UPI",    status: "Paid" },
  { date: "5 Jan 2026",  desc: "Q3 Full instalment",     amount: 14500, method: "Online", status: "Paid" },
  { date: "1 Oct 2025",  desc: "Q2 Full instalment",     amount: 14500, method: "Cash",   status: "Paid" },
];
...
export default function StudentDetail() {
  ...
  if (!student) {
    return (
      <div className="flex flex-col items-center justify-center min-h-64 gap-4">
        ...
        <Button onClick={() => navigate("/admin/students")}>← Back to list</Button>
      </div>
    );
  }
  ...
  const avgGrade  = Math.round(GRADES.reduce((s, g) => s + g.marks, 0) / GRADES.length);
  ...
  const presentDays = Object.values(ATT_RECORDS).filter((v) => v === "P").length;
  const workingDays = Object.values(ATT_RECORDS).filter((v) => v !== "H").length;
      ...
      <div className="bg-cobalt rounded-2xl p-6 relative overflow-hidden">
        ...
        <div className="relative flex flex-col sm:flex-row items-start sm:items-center gap-5">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="font-serif text-2xl text-white leading-tight">{student.name}</h1>
            <p className="text-cobalt-light text-sm mt-0.5">
              Class {student.class} &nbsp;·&nbsp; Roll {student.roll} &nbsp;·&nbsp; {student.gender}
            </p>
            <p className="text-white/40 text-xs mt-1 font-mono">{student.admNo}</p>
          </div>
          ...
          <div className="flex gap-6 flex-shrink-0">
            {[
              ...
              <div key={label} className="text-center">
                <div className="font-serif text-xl text-white font-semibold">{value}</div>
        ...
        {TABS.map((t) => (
          <button
            ...
            className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${
              tab === t ? "bg-cobalt text-white shadow-sm" : "text-ink/50 hover:text-ink"
            }`}
          >
            {t}
          </button>
            ...
            <Card>
              ...
              {[
                ...
                <div key={l} className="flex justify-between py-2 border-b border-ink/5 last:border-0">
                  ...
                  <span className="text-xs font-medium text-ink text-right">{v}</span>
                </div>
              ))}
              <div className="mt-2 pt-2 border-t border-ink/5">
                <div className="text-xs text-ink/40 mb-1">Address</div>
                <div className="text-xs text-ink leading-relaxed">{student.address}</div>
              </div>
            </Card>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <div className="h-2 bg-parchment rounded-full overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: `${student.attendance}%`, background: attColor }} />
                  </div>
                </div>
                <div>
                  <div className="flex justify-between mb-1">
                    <span className="text-xs text-ink/50">Avg grade</span>
                    <span className="text-xs font-semibold text-ink">{avgGrade}%</span>
                  </div>
                  <div className="h-2 bg-parchment rounded-full overflow-hidden">
                    <div className="h-full rounded-full bg-cobalt" style={{ width: `${avgGrade}%` }} />
                  </div>
                </div>
              </div>
            </Card>
          ...
          <Card className="lg:col-span-2">
            ...
            <div className="space-y-4">
              {GRADES.map((g) => {
                ...
                const color = pct >= 80 ? "#3A6B4F" : pct >= 60 ? "#B85C1A" : "#A0334A";
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <Card className="lg:col-span-2">
            ...
            <div className="grid grid-cols-7 mb-2">
              {WEEK_DAYS.map((d) => <div key={d} className="text-center text-[11px] font-semibold text-ink/30 py-1">{d}</div>)}
            </div>
              ...
              {calCells.map((day, i) => {
                ...
                return (
                  <div key={day} className="aspect-square flex items-center justify-center">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-semibold ${STATUS_COLORS[status] || "text-ink/20"}`}>
                      {day}
                    </div>
                  </div>
                );
            ...
            <div className="mt-4 pt-4 border-t border-ink/5">
              <div className="flex justify-between text-xs mb-1.5">
                <span className="text-ink/50"><strong className="text-sage">{presentDays} present</strong> out of <strong>{workingDays} working days</strong></span>
                <span className="font-bold" style={{ color: attColor }}>{Math.round((presentDays / workingDays) * 100)}%</span>
              </div>
              <div className="h-2 bg-parchment rounded-full overflow-hidden">
                <div className="h-full rounded-full" style={{ width: `${Math.round((presentDays / workingDays) * 100)}%`, background: attColor }} />
              </div>
            </div>
          </Card>
          ...
          <Card>
            ...
            {[
              ...
              ["Absent days",  Object.values(ATT_RECORDS).filter((v) => v === "A").length],
              ["Late/Leave",   Object.values(ATT_RECORDS).filter((v) => v === "L").length],
              ["Holidays",     Object.values(ATT_RECORDS).filter((v) => v === "H").length],
              ["YTD overall",  "91.7%"],
            ].map(([l, v]) => (
              <div key={l} className="flex justify-between py-2.5 border-b border-ink/5 last:border-0">
                <span className="text-xs text-ink/40">{l}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <Card className="lg:col-span-2">
            ...
            <div className="space-y-0">
              {PAYMENTS.map((p, i) => (
                <div key={i} className="flex items-center gap-3 py-3.5 border-b border-ink/5 last:border-0">
                  <div className="w-8 h-8 bg-sage-light rounded-lg flex items-center justify-center flex-shrink-0">
                    <svg className="w-3.5 h-3.5 text-sage" fill="none" viewBox="0 0 12 12" stroke="currentColor" strokeWidth={2}><path d="M6 10V2M3 5l3-3 3 3"/></svg>
                  </div>
                  <div className="flex-1">
                    <div className="text-sm font-medium text-ink">{p.desc}</div>
                    <div className="text-xs text-ink/40 mt-0.5">{p.date} · {p.method}</div>
                  </div>
                  <span className="text-sm font-semibold text-sage">{fmt(p.amount)}</span>
          ...
          <Card>
            ...
            {[
              ...
              ["Fee status",   <Badge key="f" variant={FEE_VARIANT[student.fees]}>{student.fees}</Badge>],
            ].map(([l, v]) => (
              <div key={l} className="flex justify-between items-center py-2.5 border-b border-ink/5 last:border-0">
                <span className="text-xs text-ink/40">{l}</span>
                <span className="text-sm font-medium text-ink">{v}</span>
              </div>
            ))}
            <div className="mt-4">
              ...
              <div className="h-2 bg-parchment rounded-full overflow-hidden">
                <div className="h-full bg-cobalt rounded-full" style={{ width: "73%" }} />
              </div>
            </div>
          </Card>
        </div>
          ...
          <Card className="lg:col-span-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
              {[
                ...
                <div key={l} className="py-2.5 border-b border-ink/5 last:border-0">
