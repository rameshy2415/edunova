...
import { Card, Badge, Table, Th, Td, Button, EmptyState } from "../../components/common";
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="font-serif text-2xl" style={{ color: "#1e0a3c" }}>Schools</h1>
          <p className="text-sm text-purple-400 mt-0.5">{filtered.length} of {SCHOOLS.length} schools</p>
        </div>
        <Button onClick={() => navigate("/superadmin/schools/new")} className="bg-purple-700 hover:bg-purple-800 text-white border-purple-700">
          + Onboard new school
        </Button>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Total",    value: SCHOOLS.length,                                     bg: "bg-purple-50",  text: "text-purple-700" },
          { label: "Active",   value: SCHOOLS.filter((s) => s.status === "Active").length, bg: "bg-green-50",   text: "text-green-700"  },
          { label: "Trial",    value: SCHOOLS.filter((s) => s.status === "Trial").length,  bg: "bg-yellow-50",  text: "text-yellow-700" },
          { label: "Expired",  value: SCHOOLS.filter((s) => s.status === "Expired").length,bg: "bg-red-50",     text: "text-red-700"    },
        ].map((s) => (
      ...
      <div className="bg-white border rounded-2xl overflow-hidden" style={{ borderColor: "#e8e4f5" }}>
        ...
        <div className="flex flex-wrap items-center gap-3 p-4 border-b" style={{ borderColor: "#f0eef9" }}>
          <div className="flex-1 min-w-48 flex items-center gap-2 rounded-xl px-3 py-2 border" style={{ background: "#F7F5FD", borderColor: "#e8e4f5" }}>
            <svg className="w-3.5 h-3.5 text-purple-300 flex-shrink-0" fill="none" viewBox="0 0 16 16" stroke="currentColor" strokeWidth={1.5}>
          ...
          <select value={planF} onChange={(e) => setPlanF(e.target.value)}
            ...
            {["All", "Enterprise", "Pro", "Basic"].map((o) => <option key={o}>{o}</option>)}
          </select>
          <select value={statusF} onChange={(e) => setStatusF(e.target.value)}
            className="rounded-xl px-3 py-2 text-sm outline-none cursor-pointer border"
            style={{ background: "#F7F5FD", borderColor: "#e8e4f5", color: "#4C1D95" }}>
            {["All", "Active", "Trial", "Expired", "Suspended"].map((o) => <option key={o}>{o}</option>)}
          </select>
        </div>

        {filtered.length === 0 ? <EmptyState title="No schools found" message="Adjust filters." /> : (
          <Table>
            <thead>
              <tr>
                <Th>School</Th><Th>City</Th><Th>Plan</Th>
                <Th>Students</Th><Th>Admin</Th><Th>Status</Th>
                <Th>Renews</Th><Th>Actions</Th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((s) => (
                <tr key={s.id} className="hover:bg-purple-50/30 transition-colors group">
                  <Td>
                    <div className="flex items-center gap-2.5">
                      ...
                      <Link to={`/superadmin/schools/${s.id}`} className="text-sm font-medium hover:text-purple-700 transition-colors" style={{ color: "#1e0a3c" }}>
                        {s.name}
                      </Link>
                    </div>
                  </Td>
                  ...
                  <Td>
                    <div className="text-sm" style={{ color: "#1e0a3c" }}>{s.admin}</div>
                    <div className="text-xs text-purple-300">{s.adminEmail}</div>
                  </Td>
                  <Td><Badge variant={STATUS_V[s.status]}>{s.status}</Badge></Td>
                  <Td>
                    <span className={`text-xs font-medium ${s.status === "Expired" ? "text-red-500" : "text-purple-400"}`}>
                      {s.renewsOn}
                    </span>
                  </Td>
                  <Td>
                    <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Link to={`/superadmin/schools/${s.id}`}
                        className="text-xs px-2.5 py-1 rounded-lg border border-purple-200 text-purple-500 hover:bg-purple-50 transition-colors">
                        View
                      </Link>
                      <Link to={`/superadmin/schools/${s.id}/edit`}
                        className="text-xs px-2.5 py-1 rounded-lg border border-purple-200 text-purple-500 hover:bg-purple-50 transition-colors">
                        Edit
                      </Link>
                    </div>
